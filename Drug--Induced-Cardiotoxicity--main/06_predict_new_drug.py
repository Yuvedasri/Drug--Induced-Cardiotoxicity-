"""
Step 6: Predict cardiotoxicity (hERG blocking) for a new drug candidate.
Takes a SMILES string, runs inference with the trained Causal-GAT, identifies
the causal substructure driving the prediction, and visualizes the result.

Usage:
    python 06_predict_new_drug.py --input_json example_case_input.json
    python 06_predict_new_drug.py --input_json your_case.json
"""
import sys
import os
import json
import torch
import numpy as np
from datetime import datetime
from rdkit import Chem
from rdkit.Chem import Draw
from rdkit import RDLogger
RDLogger.DisableLog('rdApp.*')

from model import LightweightGAT, build_clinical_feature_vector
from importlib import import_module

OUT_DIR = "outputs"

REQUIRED_HISTORY_FIELDS = ("diabetes", "hypertension", "heart_failure")
REQUIRED_LAB_FIELDS = ("potassium", "troponin", "sodium", "magnesium")
ALLOWED_SEX_VALUES = {"male", "female", "other"}


def load_case_input(input_path: str) -> dict:
    """Load a single structured case input from JSON."""
    with open(input_path, 'r', encoding='utf-8') as f:
        payload = json.load(f)

    if not isinstance(payload, dict):
        raise ValueError("Case input must be a JSON object")

    required_top_level = ("drug_name", "smiles", "age", "sex", "medical_history", "current_medication", "lab_report")
    missing = [field for field in required_top_level if field not in payload]
    if missing:
        raise ValueError(f"Case input is missing required field(s): {', '.join(missing)}")

    if not str(payload['drug_name']).strip():
        raise ValueError("'drug_name' must be a non-empty string")

    if not str(payload['smiles']).strip():
        raise ValueError("'smiles' must be a non-empty string")

    if not isinstance(payload['age'], (int, float)) or payload['age'] <= 0:
        raise ValueError("'age' must be a positive number")

    sex = str(payload['sex']).strip().lower()
    if sex not in ALLOWED_SEX_VALUES:
        raise ValueError("'sex' must be one of: male, female, other")

    if not isinstance(payload['medical_history'], dict):
        raise ValueError("'medical_history' must be a JSON object")
    missing_history = [field for field in REQUIRED_HISTORY_FIELDS if field not in payload['medical_history']]
    if missing_history:
        raise ValueError(f"'medical_history' is missing required field(s): {', '.join(missing_history)}")
    for field in REQUIRED_HISTORY_FIELDS:
        if not isinstance(payload['medical_history'][field], bool):
            raise ValueError(f"'medical_history.{field}' must be true or false")

    if not str(payload['current_medication']).strip():
        raise ValueError("'current_medication' must be a non-empty string")

    if not isinstance(payload['lab_report'], dict):
        raise ValueError("'lab_report' must be a JSON object")
    missing_labs = [field for field in REQUIRED_LAB_FIELDS if field not in payload['lab_report']]
    if missing_labs:
        raise ValueError(f"'lab_report' is missing required field(s): {', '.join(missing_labs)}")
    for field in REQUIRED_LAB_FIELDS:
        value = payload['lab_report'][field]
        if not isinstance(value, (int, float)):
            raise ValueError(f"'lab_report.{field}' must be a number")

    payload['sex'] = sex

    metadata = {
        'drug_name': payload.get('drug_name'),
        'age': payload.get('age'),
        'sex': payload.get('sex'),
        'medical_history': payload.get('medical_history', {}),
        'current_medication': payload.get('current_medication'),
        'lab_report': payload.get('lab_report', {}),
    }

    payload['case_metadata'] = metadata
    return payload


def predict(smiles: str, case_metadata: dict | None = None) -> dict:
    """
    Predict cardiotoxicity for a given SMILES string.
    
    Args:
        smiles: SMILES representation of the molecule
        
    Returns:
        dict with keys: 'smiles', 'valid', 'probability', 'classification',
        'confidence', 'causal_atoms', 'causal_drop', 'random_drop', 
        'visualization_path', 'error'
    """
    result = {
        'smiles': smiles,
        'case_metadata': case_metadata or {},
        'valid': False,
        'graph_probability': None,
        'probability': None,
        'classification': None,
        'confidence': None,
        'causal_atoms': None,
        'causal_drop': None,
        'random_drop': None,
        'visualization_path': None,
        'report_path': None,
        'input_path': None,
        'error': None
    }
    
    # Validate SMILES
    mol = Chem.MolFromSmiles(smiles)
    if mol is None:
        result['error'] = "Invalid SMILES string - RDKit cannot parse this molecule"
        return result
    
    try:
        # Sanitize molecule
        Chem.SanitizeMol(mol)
    except Exception as e:
        result['error'] = f"RDKit sanitization failed: {str(e)}"
        return result
    
    if mol.GetNumAtoms() == 0:
        result['error'] = "Molecule has no atoms"
        return result
    
    result['valid'] = True
    canonical_smiles = Chem.MolToSmiles(mol)
    
    # Check if model exists
    model_path = f"{OUT_DIR}/best_gat_model.pt"
    if not os.path.exists(model_path):
        result['error'] = f"Trained model not found at {model_path}. Please run 03_train.py first."
        return result
    
    # Import graph construction module
    try:
        graph_mod = import_module("02_graph_construction")
    except Exception as e:
        result['error'] = f"Failed to import graph construction module: {str(e)}"
        return result
    
    # Convert to graph
    data = graph_mod.mol_to_graph(canonical_smiles, label=0)  # dummy label
    if data is None:
        result['error'] = "Failed to convert molecule to graph"
        return result

    
    # Load model
    device = torch.device("cpu")
    node_dim = data.x.shape[1]
    edge_dim = data.edge_attr.shape[1]
    clinical_features = build_clinical_feature_vector(case_metadata, device=device)
    
    try:
        model = LightweightGAT(node_dim=node_dim, edge_dim=edge_dim,
                                hidden_dim=64, heads=4, num_layers=3, dropout=0.2).to(device)
        model.load_state_dict(torch.load(model_path, map_location=device, weights_only=False))
        model.eval()
    except Exception as e:
        result['error'] = f"Failed to load model: {str(e)}"
        return result
    
    # Run prediction
    data = data.to(device)
    batch = torch.zeros(data.x.size(0), dtype=torch.long, device=device)
    
    with torch.no_grad():
        graph_logits = model(data.x, data.edge_index, data.edge_attr, batch)
        graph_prob = torch.sigmoid(graph_logits).item()
        logits = model(
            data.x,
            data.edge_index,
            data.edge_attr,
            batch,
            clinical_features=clinical_features,
        )
        prob = torch.sigmoid(logits).item()
    
    result['graph_probability'] = graph_prob
    result['probability'] = prob
    result['classification'] = "TOXIC (hERG blocker risk)" if prob >= 0.5 else "NON-TOXIC"
    
    # Determine confidence
    if prob < 0.2 or prob > 0.8:
        result['confidence'] = "high confidence"
    elif 0.4 <= prob <= 0.6:
        result['confidence'] = "borderline"
    else:
        result['confidence'] = "moderate confidence"
    
    # Import causal analysis functions
    try:
        causal_mod = import_module("04_causal_analysis")
        get_node_attention = causal_mod.get_node_attention
        masked_predict = causal_mod.masked_predict
    except Exception as e:
        result['error'] = f"Failed to import causal analysis module: {str(e)}"
        return result
    
    # Identify causal substructure
    n_atoms = data.x.size(0)
    TOP_FRAC = 0.25
    k = max(1, int(np.ceil(TOP_FRAC * n_atoms)))
    
    try:
        node_scores = get_node_attention(model, data, device)
        top_idxs = np.argsort(-node_scores)[:k]
        
        # Calculate causal necessity
        orig_prob = masked_predict(model, data, [], device, clinical_features=clinical_features)
        attn_masked_prob = masked_predict(model, data, top_idxs, device, clinical_features=clinical_features)
        causal_drop = orig_prob - attn_masked_prob
        
        # Compare to random baseline
        rng = np.random.RandomState(42)
        random_drops = []
        for _ in range(5):
            rand_idxs = rng.choice(n_atoms, size=k, replace=False)
            rand_prob = masked_predict(model, data, rand_idxs, device, clinical_features=clinical_features)
            random_drops.append(orig_prob - rand_prob)
        random_drop = float(np.mean(random_drops))
        
        result['causal_drop'] = causal_drop
        result['random_drop'] = random_drop
        
        # Get atom details
        causal_atoms = []
        for idx in top_idxs:
            atom = mol.GetAtomWithIdx(int(idx))
            causal_atoms.append({
                'index': int(idx),
                'symbol': atom.GetSymbol(),
                'aromatic': atom.GetIsAromatic()
            })
        result['causal_atoms'] = causal_atoms
        
    except Exception as e:
        result['error'] = f"Causal analysis failed: {str(e)}"
        return result
    
    # Generate visualization
    try:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        smiles_hash = abs(hash(canonical_smiles)) % 10000
        filename = f"prediction_{timestamp}_{smiles_hash}.png"
        filepath = os.path.join(OUT_DIR, filename)
        
        # Highlight causal atoms
        highlight_atoms = [int(idx) for idx in top_idxs]
        highlight_colors = {int(idx): (1.0, 0.6, 0.2) for idx in top_idxs}  # orange
        
        drawer = Draw.MolDraw2DCairo(800, 600)
        drawer.DrawMolecule(mol, highlightAtoms=highlight_atoms, 
                             highlightAtomColors=highlight_colors)
        drawer.FinishDrawing()
        drawer.WriteDrawingText(filepath)
        
        result['visualization_path'] = filepath
        
    except Exception as e:
        result['error'] = f"Visualization failed: {str(e)}"
        # Don't return here - visualization is optional, rest of prediction succeeded
    
    return result


def save_prediction_report(result: dict, output_dir: str = OUT_DIR) -> str:
    """Save a JSON report with the clinical inputs and model outputs."""
    os.makedirs(output_dir, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    smiles_hash = abs(hash(result['smiles'])) % 10000
    filename = f"prediction_{timestamp}_{smiles_hash}.json"
    filepath = os.path.join(output_dir, filename)

    report = {
        'timestamp': timestamp,
        'input_path': result.get('input_path'),
        'smiles': result.get('smiles'),
        'case_metadata': result.get('case_metadata', {}),
        'valid': result.get('valid'),
        'probability': result.get('probability'),
        'graph_probability': result.get('graph_probability'),
        'classification': result.get('classification'),
        'confidence': result.get('confidence'),
        'causal_atoms': result.get('causal_atoms'),
        'causal_drop': result.get('causal_drop'),
        'random_drop': result.get('random_drop'),
        'visualization_path': result.get('visualization_path'),
        'error': result.get('error'),
    }

    with open(filepath, 'w', encoding='utf-8') as f:
        json.dump(report, f, indent=2)

    return filepath


def print_results(result: dict):
    """Print prediction results in a clean, readable format."""
    print("\n" + "=" * 60)
    print("CARDIOTOXICITY PREDICTION")
    print("=" * 60)
    if result.get('case_metadata'):
        meta = result['case_metadata']
        print(f"Drug name: {meta.get('drug_name') or 'N/A'}")
        print(f"Age: {meta.get('age') or 'N/A'}")
        print(f"Sex: {meta.get('sex') or 'N/A'}")
        history = meta.get('medical_history', {})
        print(
            "Medical history: "
            f"diabetes={history.get('diabetes', False)}, "
            f"hypertension={history.get('hypertension', False)}, "
            f"heart_failure={history.get('heart_failure', False)}"
        )
        print(f"Current medication: {meta.get('current_medication') or 'N/A'}")
        labs = meta.get('lab_report', {})
        print(
            "Lab report: "
            f"potassium={labs.get('potassium') or 'N/A'}, "
            f"troponin={labs.get('troponin') or 'N/A'}, "
            f"sodium={labs.get('sodium') or 'N/A'}, "
            f"magnesium={labs.get('magnesium') or 'N/A'}"
        )
    print(f"Input SMILES: {result['smiles']}")
    
    if not result['valid'] or result['error']:
        print(f"\nERROR: {result['error']}")
        print("=" * 60)
        return
    
    print(f"\nPredicted toxicity probability: {result['probability']:.4f}")
    if result.get('graph_probability') is not None:
        print(f"Graph-only probability: {result['graph_probability']:.4f}")
    print(f"Classification: {result['classification']} ({result['confidence']})")
    
    if result['causal_atoms']:
        print(f"\nCausal substructure driving this prediction:")
        atom_list = ", ".join([f"Atom {a['index']} ({a['symbol']})" 
                               for a in result['causal_atoms']])
        print(f"  - Atoms: {atom_list}")
        print(f"  - Masking these atoms drops toxicity probability by {result['causal_drop']:.4f}")
        print(f"  - Random-atom masking baseline drops probability by {result['random_drop']:.4f}")
        print(f"  - Causal necessity gap: {result['causal_drop'] - result['random_drop']:.4f}")
    
    if result['visualization_path']:
        print(f"\nVisualization saved to: {result['visualization_path']}")
    if result.get('report_path'):
        print(f"Prediction report saved to: {result['report_path']}")
    
    print("=" * 60 + "\n")


def main():
    """Main entry point for command-line usage."""
    import argparse

    parser = argparse.ArgumentParser(description="Predict cardiotoxicity from one structured case JSON input")
    parser.add_argument("--input_json", required=True, help="Path to a JSON file containing smiles and all clinical details")

    args = parser.parse_args()

    try:
        case_input = load_case_input(args.input_json)
    except Exception as e:
        print(f"ERROR: {e}")
        sys.exit(1)

    smiles = case_input['smiles']
    case_metadata = case_input['case_metadata']
    
    # Ensure output directory exists
    os.makedirs(OUT_DIR, exist_ok=True)
    
    # Run prediction
    result = predict(smiles, case_metadata=case_metadata)
    result['input_path'] = args.input_json

    # Save JSON report for this prediction
    try:
        result['report_path'] = save_prediction_report(result)
    except Exception as e:
        result['error'] = result['error'] or f"Failed to save prediction report: {e}"
    
    # Print results
    print_results(result)
    
    # Exit with appropriate code
    if not result['valid'] or result['error']:
        sys.exit(1)
    else:
        sys.exit(0)


if __name__ == "__main__":
    main()
